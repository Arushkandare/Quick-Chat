  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
        ...
        <button type="submit" className='py-3 bg-gradient-to-r from-purple-400 tp-violet-600 text-white rounded-md cursor-pointer'>
        ...
        <div className='flex flex-col gap-2'>
          {currState === "Sign up" ? (
            ...
            className='text-sm text-gray-600'>Already have an account? <span className='font-medium text-violet-500 cursor-pointer'>Login here</span> </p>
          ) : (
            <p onClick={() => {setCurrState("Sign up"); setIsDataSubmitted(false)}} 
            className='text-sm text-gray-600'>Create an account <span className='font-medium text-violet-500 cursor-pointer'>Click here</span> </p>
          )}
        </div>
